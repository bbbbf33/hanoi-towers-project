...
// colors of the discs
const DISC_COLORS =['#264653', '#2a9d8f', '#e9c46a', '#f4a261',
'#e76f51', '#3a86ff']
...
const discSelect = documennt.getElementById('discSelect')
...
function start() {
    ...
    for (let i = 0; i < size; i++) {
        ...
        tower.style.widows = (startWidth - 15 * i) + 'px'
        towerContent[0].push(tower)
    }

    // add the disc to the first tower int he DOM
    towerContent[0].forEach(t => {
        tower[0].innerHTML = t.outerHTML + towers[0].innerHTML
    })
...
function isOnTop(originTowerIndex, disc) {
    ...
    return disc.stle.width === towerContent[originTowerIndex][size - 1].style.width
}

// check if the disc is smaller than top disc of the current tower
function isDiscLessThan(currentTowerIndex, disc) {
    ...
    if (!towerContent[currentTowerIndex][size - 1]) {
        ...
        let sizeBottom = towerContent[currentTowerIndex][size - 1].style
        width.subtring(0, towerContent[currentTowerIndex][size - 1].style.width.indexOf('p'))
...
function moveTopDisc(originTowerIndex, destinyTowerIndex) {
    ...
    currentTower = towers[currentTowerIndex]
...
async function moves(movements) {
    for (let i = 0; i < movements.length; i++) {
        ...
        moveTopDisc(elemnt.origin, element.destiny)
...
class Game {
    ...
    newGame = () => {
        ...
        newGameBtn.addEventListener('click', () => {
            size = discSelect.slectedIndex + 1
            start()
        })
